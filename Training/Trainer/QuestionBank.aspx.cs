...
namespace Training.Trainer
{
    ...
    string courseID)
        ...
        {
            string sql =
                "SELECT QB.QuestionID,QB.CourseID,CM.CourseName,QB.TopicID,TM.TopicName,QB.Question,QB.DifficultyLevel,QB.Marks,QB.QuestionType,QB.CreatedOn FROM QuestionBank QB INNER JOIN CourseMaster CM ON QB.CourseID=CM.CourseID INNER JOIN TopicMaster TM ON QB.TopicID=TM.TopicID WHERE QB.QuestionOwnerType='Trainer' AND QB.OwnerID = @OwnerID";
    ...
    EventArgs e)
        ...
        {
            string sql =
                "SELECT ISNULL(MAX(CAST(RIGHT(QuestionID, 6)AS INT)),0) + 1 FROM QuestionBank";
...
"000000");
        ...
        {
            string sql =
                "SELECT COUNT(*) FROM QuestionBank WHERE CourseID = @CourseID AND TopicID = @TopicID AND Question = @Question AND QuestionOwnerType = 'Trainer' AND OwnerID = @OwnerID";
            ...
            SqlParameter[] param =
{
    ...
    new SqlParameter("@OwnerID", Session["UserID"]?.ToString())
};
    ...
    EventArgs e)
        ...
        private void UpdateQuestion()
        {
            ...
            sql +=
                " WHERE QuestionID = @QuestionID AND OwnerID = @OwnerID AND QuestionOwnerType = 'Trainer'";
    ...
    string questionID)
        {
            string sql =
                "DELETE FROM QuestionBank WHERE QuestionID = @QuestionID AND OwnerID = @OwnerID AND QuestionOwnerType = 'Trainer'";
    ...
    string questionID)
        {
            string sql =
                "SELECT *  FROM QuestionBank WHERE QuestionID = @QuestionID AND OwnerID = @OwnerID AND QuestionOwnerType = 'Trainer'";
